   ...
   # skip_before_action :verify_authenticity_token
    ...
    def create
      if !current_user.admin? && current_user.form_partecipazione_inviato? && !current_user.officialized?
	      render :file => "/decidim_application/app/views/decidim/form_partecipazione/show_modal2.js.erb"
      else
        if current_user.admin? || current_user.officialized?
          ...
          #render :show_modal
          render :file => "/decidim_application/app/views/decidim/form_partecipazione/show_modal.js.erb"
        end
      end
    end
